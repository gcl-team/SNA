...
public class StateDurationMetric<TState> where TState : Enum
{
    ...
    /// <param name="initialTime">The simulation time at which tracking begins. Defaults to 0.</param>
    ...
    public double GetProportionOfTimeInState(TState stateToQuery, long asOfClockTime)
    {
        ...
        // If querying for a historical time, we MUST use the history list for an accurate calculation.
        if (IsHistoryEnabled && asOfClockTime < CurrentTime)
        {
            double durationInState = 0;
            for (int i = 0; i < _history.Count - 1; i++)
            {
                var intervalStartTime = _history[i].Time;
                var intervalState = _history[i].State;
                var intervalEndTime = _history[i + 1].Time;

                if (intervalStartTime >= asOfClockTime)
                {
                    // We've gone past our target time, no more intervals will contribute.
                    break;
                }
