...
public class StateDurationMetricTests
{
    ...
    public void Constructor_NegativeInitialTime_ThrowsArgumentOutOfRangeException()
    {
        ...
        Assert.Throws<ArgumentOutOfRangeException>(() => 
            new StateDurationMetric<TestMachineState>(initialState, -1)
        );
    }
    ...
    public void UpdateState_MultipleTransitions_AccumulatesDurationsCorrectly()
    {
        ...
        // Act
        metric.UpdateState(TestMachineState.Busy, 10);        // Idle for 10s
        metric.UpdateState(TestMachineState.Maintenance, 15); // Busy for 5s
        metric.UpdateState(TestMachineState.Idle, 25);        // Maintenance for 10s

        // Assert
        AssertHelpers.AreEqual(25, metric.CurrentTime);
        Assert.Equal(TestMachineState.Idle, metric.CurrentState);

        AssertHelpers.AreEqual(10, metric.StateDurations[TestMachineState.Idle]);
        AssertHelpers.AreEqual(5, metric.StateDurations[TestMachineState.Busy]);
        AssertHelpers.AreEqual(10, metric.StateDurations[TestMachineState.Maintenance]);
    }
    ...
    public void UpdateState_TransitionToSameStateAtLaterTime_AddsToDuration()
    {
        ...
        metric.UpdateState(TestMachineState.Idle, 5); // Idle for 5s

        // Assert
        AssertHelpers.AreEqual(5, metric.CurrentTime);
        Assert.Equal(TestMachineState.Idle, metric.CurrentState);
        Assert.True(metric.StateDurations.ContainsKey(TestMachineState.Idle));
    ...
    {
        // Arrange
        var metric = new StateDurationMetric<TestMachineState>(TestMachineState.Idle, 5);

        // Act
        metric.UpdateState(TestMachineState.Busy, 5); // Transition at the exact same time
    ...
    {
        // Arrange
        var metric = new StateDurationMetric<TestMachineState>(TestMachineState.Idle, 10);

        // Act & Assert
        Assert.Throws<ArgumentOutOfRangeException>(() => 
            metric.UpdateState(TestMachineState.Busy, 5)
        );
    }
    ...
    {
        // Arrange
        var metric = new StateDurationMetric<TestMachineState>(TestMachineState.Idle, 0, enableHistory: true);

        // Act
        metric.UpdateState(TestMachineState.Busy, 10);
        metric.UpdateState(TestMachineState.Maintenance, 15);
    ...
    public void WarmedUp_NegativeClockTime_ThrowsArgumentOutOfRangeException()
    {
        ...
        // Act & Assert
        Assert.Throws<ArgumentOutOfRangeException>(() => metric.WarmedUp(-5));
    }
    ...
    {
        // Arrange
        var metric = new StateDurationMetric<TestMachineState>(TestMachineState.Idle, 0); // InitialTime = 0
        metric.UpdateState(TestMachineState.Busy, 10);                                    // Idle for 10s
        metric.UpdateState(TestMachineState.Maintenance, 15);                             // Busy for 5s
        // CurrentState = Maintenance, CurrentTime = 15
        // StateDurations: { Idle: 10, Busy: 5 }

        var asOfTime = 20L; // Total observed time is 20s (from 0 to 20)
    ...
    public void GetProportionOfTime_ForCurrentState_IncludesOngoingDuration()
    {
        // Arrange
        var metric = new StateDurationMetric<TestMachineState>(TestMachineState.Idle, 0);
        metric.UpdateState(TestMachineState.Busy, 10); // CurrentTime=10, CurrentState=Busy
        var asOfTime = 25L; // Total time is 25s.
    ...
    public void GetProportionOfTime_ForCompletedState_CalculatesCorrectly()
    {
        // Arrange
        var metric = new StateDurationMetric<TestMachineState>(TestMachineState.Idle, 0); // InitialTime = 0
        metric.UpdateState(TestMachineState.Busy, 10); // Idle ran for 10s. CurrentTime = 10.
        var asOfTime = 20L; // Total time is 20s.
    ...
    public void GetProportionOfTimeInState_AsOfTimeEqualsCurrentTime_CorrectProportion()
    {
        // Arrange
        var metric = new StateDurationMetric<TestMachineState>(TestMachineState.Idle, 0); // InitialTime = 0
        metric.UpdateState(TestMachineState.Busy, 10);  // Idle for 10s. CurrentTime=10, CurrentState=Busy
        // StateDurations: { Idle: 10 }

        var asOfTime = 10L; // Same as CurrentTime
    ...
    public void GetProportionOfTimeInState_AsOfTimeBeforeInitialTime_ThrowsArgumentOutOfRangeException()
    {
        ...
        // Act & Assert
        Assert.Throws<ArgumentOutOfRangeException>(() => metric.GetProportionOfTimeInState(TestMachineState.Idle, 5));
    }
    ...
    public void GetProportionOfTime_AsOfTimeBeforeWarmupTime_ThrowsException()
    {
        // Arrange
        var metric = new StateDurationMetric<TestMachineState>(TestMachineState.Idle, 0);
        metric.UpdateState(TestMachineState.Busy, 50);

        metric.WarmedUp(100); // New InitialTime is 100

        // Act & Assert
        var ex = Assert.Throws<ArgumentOutOfRangeException>(() =>
            metric.GetProportionOfTimeInState(TestMachineState.Busy, 90) // Querying before InitialTime
        );
    ...
    {
        // Arrange
        var metric = new StateDurationMetric<TestMachineState>(TestMachineState.Idle, 5); // InitialTime = 5

        // Act
        // Querying at the exact initial time.
        var propIdle = metric.GetProportionOfTimeInState(TestMachineState.Idle, 5);
        var propBusy = metric.GetProportionOfTimeInState(TestMachineState.Busy, 5);
    ...
    public void GetProportionOfTime_ForPastTimestampWithHistory_CalculatesCorrectly()
    {
        ...
        metric.UpdateState(TestMachineState.Busy, 10); // Idle duration: 10s (from t=0 to t=10)
        metric.UpdateState(TestMachineState.Idle, 20); // Busy duration: 10s (from t=10 to t=20)

        var asOfTime = 15; // We are querying for a time in the past.
