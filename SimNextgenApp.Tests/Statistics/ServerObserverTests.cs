...
public class ServerObserverTests
{
    ...
    [Fact]
    public void Constructor_SubscribesToActions_Correctly()
    {
        // Arrange
        var server = CreateTestServer();
        int initialDepartActions = server.LoadDepartActions.Count;
        int initialStateChangeActions = server.StateChangeActions.Count;

        // Act
        var observer = new ServerObserver<DummyLoad>(server);

        // Assert
        Assert.Equal(initialDepartActions + 1, server.LoadDepartActions.Count);
        Assert.Equal(initialStateChangeActions + 1, server.StateChangeActions.Count);
    }
